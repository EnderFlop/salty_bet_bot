...
POVERTY_LIMIT = 500 #amount of money that, if below, always all in.
...
def start_saltybet():
  ...
  while True:
    ...
    if balance > past_balance:
      print(f"{loop}. WON! Made ${balance - past_balance}")
    elif balance < past_balance:
      print(f"{loop}. LOST... Lost ${past_balance - balance}")

    #START OF BETTING LOOP

    loop += 1
    print(f"{loop}. Fight Number {loop}")
    ...
    if balance < POVERTY_LIMIT: #Since the floor is around $200, always go all in until you get out of poverty.
      to_bet = balance
      print(f"{loop}. Odds Are {one}% vs {two}%. Current balance is ${balance}. BELOW POVERTY LIMIT! To Bet: ${to_bet}")
    else:
      ...
      print(f"{loop}. Odds Are {one}% vs {two}%. Engine gives {percentage_bet}% of balance to bet. Current balance is ${balance}. To Bet: ${to_bet}")
    ...
    time.sleep(1) #can't use implicit wait because they already exist, just have to wait after wager closes for it to update.
    lst = driver.find_elements(By.XPATH, "//span[@id='lastbet']/span")
    to_gain, odds_1, odds_2 = lst[1].text, lst[2].text, lst[3].text
    if one > two: #add some flair to the chosen fighter's odds.
      odds_1 = f"*{odds_1}*"
    else:
      odds_2 = f"*{odds_2}*"
    print(f"{loop}. Odds are {odds_1}:{odds_2}. ${to_bet} -> {to_gain} potential")
